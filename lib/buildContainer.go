...
func BuildContainer(repoUrl string, buildID uint, cont db.Container, wg *sync.WaitGroup, failed *bool) {
	...
	DockerCli.ContainerRemove(context.TODO(), mainContainerResponse.ID, types.ContainerRemoveOptions{Force: true})
	if cont.ServiceImage != nil {
		DockerCli.ContainerRemove(context.TODO(), serviceContainerResponse.ID, types.ContainerRemoveOptions{Force: true})
	}
