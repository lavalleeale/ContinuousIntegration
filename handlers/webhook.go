			...
			if *event.Action == "rerequested" {
				...
				if err != nil {
					if strings.Contains(err.Error(), "No file named") {
						...
						client.Checks.UpdateCheckRun(context.TODO(), *event.Repo.Owner.Login, *event.Repo.Name, *event.CheckRun.ID, github.UpdateCheckRunOptions{Conclusion: &conclusion, Name: "Test", Output: &github.CheckRunOutput{Title: &title, Summary: &summary}})
						return
					} else {
						// This should never fail since the webhook is directly from github and we know that our installation must be valid, and the panic will not be facing users
						panic(err)
